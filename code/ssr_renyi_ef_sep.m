function Xk = ssr_renyi_ef_sep( Y, A1, A2, par, lambda, mu)
	...
	for iter = 1 : maxiter
		...
		Xk_r = Yk;
        Xkp1_r = Xk_r;
        fpre = kappa/2*norm(Xkp1_r-Gk, 'fro')^2+lambda*fun_compute(Xkp1_r(1:col_A1), p, alpha)+lambda*mu*fun_compute(Xkp1_r(col_A1+1:end), p, alpha);
		for inneriter  = 1:innermaxiter
			w_1 = fun_sg(Xk_r(1:col_A1), p, alpha, epsilon);
			Xkp1_r_1 = weighted_shrinkage(Gk(1:col_A1), lambda/kappa, w_1);

            w_2 = fun_sg(Xk_r(col_A1+1:end), p, alpha, epsilon);
            Xkp1_r_2 = weighted_shrinkage(Gk(col_A1+1:end), lambda*mu/kappa, w_2);

            Xkp1_r = [Xkp1_r_1; Xkp1_r_2];

            fcur =  kappa/2*norm(Xkp1_r-Gk, 'fro')^2+lambda*fun_compute(Xkp1_r(1:col_A1), p, alpha)+lambda*mu*fun_compute(Xkp1_r(col_A1+1:end), p, alpha);
            if (fcur>fpre)
                Xkp1 = Xkp1_r;
                break;
            end
