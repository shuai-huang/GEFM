function Xk = srr_shannon_ef( Y, A, par, lambda)
	...
	for iter = 1 : maxiter
		...
		Xk_r = Yk;
        Xkp1_r = Xk_r;
        fpre = kappa/2*norm(Xkp1_r-Gk, 'fro')^2+lambda*fun_compute(Xkp1_r, p);
		for inneriter  = 1:innermaxiter
			w = fun_sg(Xk_r, p, epsilon);
			Xkp1_r = weighted_shrinkage(Gk, lambda/kappa, w);

            fcur = kappa/2*norm(Xkp1_r-Gk, 'fro')^2+lambda*fun_compute(Xkp1_r, p);
            if (fcur>fpre)
                Xkp1 = Xkp1_r;
                break;
            end
